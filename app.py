...
class Settings(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    company_name_tamil = db.Column(
        db.Text, default="ஸ்ரீ தனலட்சுமி புளூ மெட்டல்ஸ்")
    company_name_english = db.Column(
        db.String(200), default="Sri Dhanalakshmi Blue Metals")
    address_tamil = db.Column(
        db.Text,
        default="நெமிலி & எண்வரடி அஞ்சல், எண்டியூர்,\nவாணூர் தாலுகா, விழுப்புரம் மாவட்டம்.")
    address_english = db.Column(
        db.Text,
        default="Nemili & Envaradi Post, Endiyur,\nVandur Taluk, Villupuram District.")
    gstin = db.Column(db.String(50), default="33AUXPR8335C1Z7")
    phone_numbers = db.Column(db.String(200),
                              default="97883 88823, 97515 31619, 75026 27223")
    ...
    sms_provider = db.Column(db.String(50),
                             default="twilio")  # twilio, msg91, generic
    sms_api_key = db.Column(db.String(200), nullable=True)
    sms_api_secret = db.Column(
        db.String(200),
        nullable=True)  # For Twilio Auth Token
    sms_sender_id = db.Column(db.String(50), nullable=True)
    sms_api_url = db.Column(db.String(500),
                            nullable=True)  # For generic provider
    sms_template = db.Column(db.Text, nullable=True)
    whatsapp_provider = db.Column(
        db.String(50),
        default="twilio")  # twilio, generic
    whatsapp_sender_number = db.Column(db.String(20), nullable=True)
    whatsapp_api_key = db.Column(
        db.String(200),
        nullable=True)  # For generic provider
    whatsapp_api_url = db.Column(
        db.String(500),
        nullable=True)  # For generic provider
...
class AuditLog(db.Model):
    ...
    # login, create_bill, edit_bill, delete_bill, etc.
    action = db.Column(db.String(100), nullable=False)
    # invoice, user, customer, etc.
...
# ------------------------------------------------------------
# Routes - Registration function (called from create_app)
# ------------------------------------------------------------
def register_routes(app):
    ...
    def ensure_database():
        ...
        if not is_database_ready():
            ...
            if endpoint in {
                "static",
                "ping"} or request.path.startswith("/static"):
                return
        ...
        try:
            ...
            return render_template(
                "error.html", error=f"Database connection failed: {str(e)}. Please check your DATABASE_URL."), 500
            ...
            if request.method == "POST":
                ...
                if user and user.check_password(password):
                    if user.status != "active":
                        return render_template(
                        "login.html", error="Your account is inactive. Please contact administrator.")
    ...
    def dashboard():
        ...
        monthly_invoices = invoice_query.filter(
            Invoice.date >= month_start).all()
        ...
        recent_invoices = invoice_query.order_by(
            desc(Invoice.created_at)).limit(10).all()
        ...
        if current_user.role in ["admin", "staff"]:
            ...
            if current_user.customer_id:
                recent_customers = [
                    current_user.customer] if current_user.customer else []
            else:
                recent_customers = []
    ...
    def edit_customer(customer_id):
        try:
            ...
            customer.gst_number = request.form.get(
            "gst_number", "").strip() or None
    ...
    def api_customers():
        ...
        return jsonify([{"id": c.id,
                         ...
                         "phone": c.phone or "",
                         "address": c.address or ""} for c in customers])
        ...
        if customer_id:
            ...
            if not vehicles:
                ...
                Vehicle.vehicle_number.ilike(
                    ...
                    f"%{query}%")).order_by(
                    desc(
                    Vehicle.created_at)).limit(10).all()

                    return jsonify([{"id": v.id,
                         "vehicle_number": v.vehicle_number,
                         "vehicle_type": v.vehicle_type or ""} for v in vehicles])
                ...
                try:
                    ...
                    if not customer_name:
                        ...
                        items_data = [{"id": item.id, "name": item.name,
                                   "rate": float(item.rate)} for item in items]
                        return render_template(
                        "create_bill.html", items=items, items_data=items_data)
                        ...
                        if not vehicle_number:
                            ...
                            if not re.match(
                                ...
                                vehicle_number):
                                flash(
                                "Invalid vehicle number format. Expected format: TN32AX3344 or TN10AA9988",
                                "danger")
                                items = Item.query.filter_by(is_active=True).all() or []
                                items_data = [{"id": item.id, "name": item.name,
                                   "rate": float(item.rate)} for item in items]
                                ...
                                if not customer:
                                    customer = Customer(
                                    name=customer_name, gst_number=request.form.get(
                                    "customer_gst", "").strip() or None, phone=request.form.get(
                                    "customer_phone", "").strip() or None, address=request.form.get(
                                    "customer_address", "").strip() or None, )
                                    db.session.add(customer)
                                    db.session.flush()
                                    ...
                                    if not vehicle:
                                        ...
                                        "delivery_location", "").strip() or None
                                        has_waybill = True
                                        ...
                                        if not driver_name:
                                            ...
                                            "material_type", "").strip() or None
                                            vehicle_capacity = request.form.get(
                                            "vehicle_capacity", "").strip() or None

                                            # Calculate loading and unloading times
                                            loading_time = datetime.now()
                                            delivery_duration = request.form.get(
                                            "delivery_duration", "").strip()
                                            duration_unit = request.form.get(
                                            "duration_unit", "hours").strip()
                                                    ...
                                                    try:
                                                        ...
                                                        db.session.add(invoice_item)
                                                                            ...
                                                                            if subtotal == 0:
                                                                                db.session.rollback()
                                                                                flash(
                                                                                "At least one item with quantity and rate is required",
                                                                                "danger")
                                                                                items = Item.query.filter_by(is_active=True).all() or []
                                                                                items_data = [{"id": item.id, "name": item.name,
                                                                                "rate": float(item.rate)} for item in items]
                                                                                ...
                                                                                try:
                                                                                    ...
                                                                                    if settings_obj.auto_send_sms or settings_obj.auto_send_whatsapp:
                                                                                        ...
                                                                                        notification_results = send_invoice_notification(
                                                                                        settings_obj, invoice, base_url)

                                                                                        if settings_obj.auto_send_sms and notification_results.get(
                                                                                            "sms", {}).get("success"):
                                                                                            flash(
                                                                                                "Bill created and SMS sent successfully!", "success")
                                                                                        elif settings_obj.auto_send_whatsapp and notification_results.get("whatsapp", {}).get("success"):
                                                                                            flash(
                                                                                                "Bill created and WhatsApp sent successfully!", "success")
                                                                                        else:
                                                                                        ...
                                                                                        flash("Bill created successfully!", "success")
                                                                                                            ...
                                                                                                            flash(
                                                                                                            "Bill created successfully! (Notification sending failed)",
                                                                                                            "success")
    ...
    def invoice_pdf(invoice_id):
        ...
        if current_user.role == "user" and current_user.customer_id != invoice.customer_id:
            ...
            for line in settings.address_tamil.split("\n"):
                ...
                y -= 20
                c.drawString(
                60, y, f"தேதி / Date: {invoice.date.strftime('%d-%m-%Y')}")
                y -= 20
                ...
                # Customer details
                c.drawString(
                60, y, f"வாடிக்கையாளர் / Customer: {invoice.customer.name}")
                y -= 20
                if invoice.customer.gst_number:
                    c.drawString(60, y, f"GST No: {invoice.customer.gst_number}")
                    y -= 20
                    c.drawString(
                    60, y, f"வாகன எண் / Vehicle: {invoice.vehicle.vehicle_number if invoice.vehicle else 'N/A'}")
                    y -= 20
                    if invoice.delivery_location:
                        c.drawString(
                        60, y, f"விநியோக இடம் / Delivery Location: {invoice.delivery_location}")
                        y -= 20
                        y -= 10
                        ...
                        for item in invoice.items:
                            ...
                            if invoice.has_waybill and invoice.waybill:
                                ...
                                if invoice.waybill.driver_name:
                                    c.drawString(
                                    60, y, f"Driver Name / ஓட்டுநர் பெயர்: {invoice.waybill.driver_name}")
                                    y -= 15
                                    if invoice.waybill.material_type:
                                        c.drawString(
                                        60, y, f"Material Type / பொருள் வகை: {invoice.waybill.material_type}")
                                        y -= 15
                                        if invoice.waybill.loading_time:
                                            c.drawString(
                                            60, y, f"Loading Time / ஏற்ற நேரம்: {
                                            invoice.waybill.loading_time.strftime('%d-%m-%Y %H:%M')}")
                                            y -= 15
                                            if invoice.waybill.unloading_time:
                                                c.drawString(
                                                60, y, f"Unloading Time / இறக்கும் நேரம்: {
                                                invoice.waybill.unloading_time.strftime('%d-%m-%Y %H:%M')}")
                                                y -= 15
                                                    ...
                                                    if hours >= 1:
                                                        duration_str = f"{hours:.1f} hours"
                                                            ...
                                                            if invoice.waybill.vehicle_capacity:
                                                                c.drawString(
                                                                60, y, f"Vehicle Capacity / வாகன திறன்: {invoice.waybill.vehicle_capacity}")
                                                                y -= 15
                                                                if invoice.waybill.delivery_location:
                                                                    c.drawString(
                                                                    60, y, f"Delivery Location / விநியோக இடம்: {invoice.waybill.delivery_location}")
                                                                    y -= 15
                                                                    y -= 10
                                                                    ...
                                                                    c.setFont(font_name, 10)
                                                                    c.drawString(
                                                                    60, y, "அங்கீகரிக்கப்பட்டவர் – ஸ்ரீ தனலட்சுமி புளு மெட்டல்ஸ்")
                                                                    y -= 30
    ...
    def duplicate_invoice(invoice_id):
        try:
            ...
            for item in original.items:
                ...
                new_invoice.id,
                f"Bill {
                new_invoice.bill_no} duplicated from {
                original.bill_no}",
                request.remote_addr)
            ...
            if month == 12:
                    ...
                    for inv in invoices:
                        ...
                        if week_num not in weekly_data:
                            ...
                            weekly_totals = {
                            week: sum(
                            inv.grand_total for inv in invs) for week,
                            invs in weekly_data.items()}
        ...
        try:
            if week_str:
                ...
                start = jan1 + timedelta(days=days_offset - jan1.weekday())
                    ...
                    for inv in invoices:
                        writer.writerow([inv.bill_no,
                             inv.customer.name,
                             inv.vehicle.vehicle_number if inv.vehicle else "",
                             inv.date.strftime("%Y-%m-%d"),
                             inv.grand_total])

                        output.seek(0)
                        week_label = week_str if week_str else f"{
                        start.strftime('%Y-%m-%d')}_to_{
                        end.strftime('%Y-%m-%d')}"
                        return send_file(
    ...
    def settings():
        if request.method == "POST":
            try:
                settings_obj = get_settings()
                settings_obj.company_name_tamil = request.form.get(
                "company_name_tamil", "").strip()
                settings_obj.company_name_english = request.form.get(
                "company_name_english", "").strip()
                settings_obj.address_tamil = request.form.get(
                "address_tamil", "").strip()
                settings_obj.address_english = request.form.get(
                "address_english", "").strip()
                settings_obj.gstin = request.form.get("gstin", "").strip()
                settings_obj.phone_numbers = request.form.get(
                "phone_numbers", "").strip()
                settings_obj.cgst_percent = float(
                request.form.get("cgst_percent", 2.5))
                settings_obj.sgst_percent = float(
                request.form.get("sgst_percent", 2.5))
                settings_obj.from_location = request.form.get(
                "from_location", "நெமிலி").strip()
    ...
    def backup_database():
        ...
        data = {"customers": [{"id": c.id,
                               "name": c.name,
                               "gst_number": c.gst_number,
                               "phone": c.phone,
                               "address": c.address} for c in Customer.query.all()],
                "vehicles": [{"id": v.id,
                              "vehicle_number": v.vehicle_number,
                              "vehicle_type": v.vehicle_type} for v in Vehicle.query.all()],
                "invoices": [{"id": inv.id,
                              "bill_no": inv.bill_no,
    ...
    def toggle_item(item_id):
        try:
            ...
            flash(
            f"Item {
                'activated' if item.is_active else 'deactivated'} successfully",
            "success")
        except Exception as e:
                db.session.rollback()
    ...
    def admin_messaging():
        if request.method == "POST":
            try:
                settings = get_settings()
                # SMS Settings
                settings.sms_provider = request.form.get(
                "sms_provider", "twilio").strip()
                settings.sms_api_key = request.form.get(
                "sms_api_key", "").strip() or None
                settings.sms_api_secret = request.form.get(
                "sms_api_secret", "").strip() or None
                settings.sms_sender_id = request.form.get(
                "sms_sender_id", "").strip() or None
                settings.sms_api_url = request.form.get(
                "sms_api_url", "").strip() or None
                settings.sms_template = request.form.get(
                "sms_template", "").strip() or None
                settings.auto_send_sms = request.form.get("auto_send_sms") == "on"

                # WhatsApp Settings
                settings.whatsapp_provider = request.form.get(
                "whatsapp_provider", "twilio").strip()
                settings.whatsapp_sender_number = request.form.get(
                "whatsapp_sender_number", "").strip() or None
                settings.whatsapp_api_key = request.form.get(
                "whatsapp_api_key", "").strip() or None
                settings.whatsapp_api_url = request.form.get(
                "whatsapp_api_url", "").strip() or None
                settings.whatsapp_template = request.form.get(
                "whatsapp_template", "").strip() or None
                settings.auto_send_whatsapp = request.form.get(
                "auto_send_whatsapp") == "on"
                ...
                if not settings.sms_api_key:
                    ...
                    if settings.sms_template:
                        ...
                        if result.get("success"):
                            flash(
                            f"Test SMS sent successfully! Message ID: {
                            result.get(
                            'message_id',
                            'N/A')}",
                            "success")
            ...
            if not test_number:
                ...
                if not settings.whatsapp_sender_number:
                    ...
                    if settings.whatsapp_template:
                        ...
                        if result.get("success"):
                            flash(
                            f"Test WhatsApp sent successfully! Message ID: {
                            result.get(
                            'message_id',
                            'N/A')}",
                            "success")
                                ...
                                flash(
                                f"Failed to send test WhatsApp: {
                                result.get(
                                'error',
                                'Unknown error')}",
                                "danger")
        ...
        try:
            backup_data = {"customers": [{"id": c.id,
                       ...
                       "vehicles": [{"id": v.id,
                                     ...
                                     "customer_id": v.customer_id} for v in Vehicle.query.all()],
                       "invoices": [{"id": inv.id,
                                     "bill_no": inv.bill_no,
                                     "date": inv.date.isoformat(),
                                     "customer_id": inv.customer_id,
                       ...
                       "settings": {"company_name_tamil": get_settings().company_name_tamil,
        ...
        try:
            if 'backup_file' not in request.files:
                ...
                if file.filename == '':
                    ...
                    flash(
                    "Restore functionality - implement with caution. This will overwrite existing data.",
                    "warning")
                    return redirect(url_for("admin_panel"))
        except Exception as e:
    ...
    def ping():
        ...
        # Setup page removed - PostgreSQL is external and configured via
        # DATABASE_URL
    ...
    def handle_exception(err):
        ...
        print("⚠️ Error:", err)
        traceback.print_exc()
        message = "An error occurred. Please try again." if RUNNING_ON_VERCEL else str(
            err)
...
def create_app():
    ...
    app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(
        hours=8)  # 8 hour session timeout
